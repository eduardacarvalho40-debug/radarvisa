...
function extrairPublicacoes(html, dataExib) {
  ...
  var termosHPPC = [
    ...
    "hppc", "2731", "registro de produto cosmetico"
  ];
...
function processar(item, dataExib, termosHPPC, vistos, resultados) {
  ...
  // O orgao vem como "Ministerio da Saude/Agencia Nacional de Vigilancia Sanitaria/..."
  ...
  // Filtra: precisa ser HPPC
  // Normaliza o texto para comparacao sem acentos
  var textoNorm = textoCompleto.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  var ehHPPC = termosHPPC.some(function(t) { return textoNorm.indexOf(t) !== -1; });
  if (!ehHPPC) {
    // Log dos primeiros 3 nao-HPPC para debug
    if (resultados.length === 0 && Object.keys(vistos).length < 3) {
      console.log("NAO-HPPC:", titulo.substring(0,60));
    }
    return;
  }
...
function classificar(titulo, resumo) {
  ...
  if (t.includes("2731") || (t.includes("registro") && (t.includes("concess") || t.includes("produto cosmet")))) return "Registro Novo";
  ...
  if (t.includes("2112") || t.includes("inclusao") || t.includes("inclusão")) return "Inclusao Apresentacao";
