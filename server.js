...
const server = http.createServer(function(req, res) {
  ...
  if (req.url === "/api/claude" && req.method === "POST") {
    if (!CHAVE) {
      ...
      res.end(JSON.stringify({ error: "SEM_CHAVE", mensagem: "Configure ANTHROPIC_API_KEY no Railway" }));
      return;
    }
    ...
    req.on("end", function() {
      ...
      // Garante modelo correto e max_tokens valido
      parsed.model = "claude-sonnet-4-6";
      if (!parsed.max_tokens || parsed.max_tokens > 8096) {
        parsed.max_tokens = 8096;
      }

      // Garante que tools esta no formato correto
      if (parsed.tools) {
        parsed.tools = parsed.tools.map(function(t) {
      ...
      const proxy = https.request(opcoes, function(apiRes) {
        ...
        apiRes.on("end", function() {
          if (apiRes.statusCode !== 200) {
            console.log("Erro API Anthropic " + apiRes.statusCode + ":", dados.substring(0, 500));
          }
...
server.listen(PORT, "0.0.0.0", function() {
  ...
  console.log("Chave API: " + (CHAVE ? "OK (" + CHAVE.substring(0,10) + "...)" : "NAO ENCONTRADA"));
});
