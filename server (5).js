...
function processar(item, dataExib, termosHPPC, vistos, resultados) {
  ...
  // Normaliza sem acentos para comparacao
  var textoNorm = textoCompleto.normalize("NFD").replace(/[\u0300-\u036f]/g, "");

  // Exclui saneantes e outras areas que nao sao HPPC
  var ehExcluido = termosExcluir.some(function(t) { return textoNorm.indexOf(t) !== -1; });
  if (ehExcluido) return;

  // Verifica se e HPPC — primeiro pelas frases exatas (mais preciso)
  var ehHPPC = frasesExatas.some(function(t) { return textoNorm.indexOf(t) !== -1; });

  // Se nao achou pelas frases, tenta pelos termos gerais
  if (!ehHPPC) {
    ehHPPC = termosHPPC.some(function(t) { return textoNorm.indexOf(t) !== -1; });
  }
