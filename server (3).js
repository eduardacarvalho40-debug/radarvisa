...
function processar(item, dataExib, termosHPPC, vistos, resultados) {
  ...
  var orgao   = (item.orgaoName || item.orgao || item.hierarchyStr || "").toLowerCase();
  ...
  if (!textoCompleto.includes("anvisa")) return;

  // Filtra: precisa ser HPPC
  var ehHPPC = termosHPPC.some(function(t) { return textoCompleto.indexOf(t) !== -1; });
  if (!ehHPPC) return;
